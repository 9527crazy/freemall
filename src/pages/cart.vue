						...
						<ul class="cart-item-list">
							<li v-for="item in cartList" :key="item.productId">
								...
								<div class="cart-tab-4">
									<div class="item-price-total">{{(item.productPrice*item.productNum) | capitalize}}</div>
								</div>
                    ...
                    <use xlink:href="#icon-ok" /></svg>
						...
						<div class="cart-foot-r">
							<div class="item-total">
								总价: <span class="total-price">{{totalPrice | capitalize}}</span>
							</div>
							<div class="btn-wrap">
...
export default{
	...
	computed:{
		//当数组中所有对象都返回true的时候，我们整体才返回true
		checkAllFlag(){
	...
	methods:{
		...
		editCart(type,item){
			if (type=='add'){
				item.productNum++;
			}if (type=='minus' &&item.productNum>0) {
				item.productNum--;
			}if (type =='checked'){
